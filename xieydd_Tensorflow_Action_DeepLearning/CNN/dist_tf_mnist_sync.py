#-*- coding: utf-8 -*-
#@Description: CNN 7层网络对MNIST的训练过程实现 计算图内的分布式模型即同步
#@author xieydd xieydd@gmail.com  
#@date 2017-10-25 下午13:47:29
import time
...
def build_model(x,y_,is_chief):
    ...
    opt = tf.train.SyncReplicasOptimizer(
        ...
        #定义每一轮更新需要多少计算服务器得出梯度
        replicas_to_aggregate = n_workers,
        #指定当前计算服务器的编号
        replicas_id = FLAGS.task_id
    )    
    
    triain_op = opt.minimize(loss,global_step=global_step)
...
def main(argv=None):
    ...
    with tf.device(tf.train.replica_device_setter(worker_device="/job:worker/task:%d" % FLAGS.task_id,cluster=cluster)):
        ...
        #开始训练之前，主计算服务器需要启动协调同步更新的队列进行初始化操作
        if is_chief:
